...
* @verbatim
* ==============================================================================
*  该固件由esp-idf 的esp_hid_device例程魔改而来
 * 开发环境 Clion
 * idf版本 2022-11-04的master (如果有问题可以试试 release v5
*  蓝牙协议走的是ble，底层api idf已经给我们写好了，比如这个 esp_hidd_dev_input_set，可以直接发送
*  分为三个任务
*  1.按键检测任务，检测是否被按下
*  2.灯光任务
*  3.发送hid报文任务
...
static const char *TAG = "pansy's keyboard";
...
const unsigned char hidapiReportMap[] = { //8 bytes input, 8 bytes feature
        ...
        0xB2, 0x02,
        0x01,  //   Feature (Data,Var,Abs,No Wrap,Linear,Preferred State,No Null Position,Non-volatile,Buffered Bytes)
        0x95, 0x08,        //   Report Count (8)
...
static esp_hid_device_config_t ble_hid_config = {
        ...
        .product_id         = 0x05DF,
        .version            = 0x0100,
        .device_name        = "pansy's keyboard",
        .manufacturer_name  = "ACE",
...
#define HID_RPT_ID_CC_IN        0   // Consumer Control input report ID  3
#define HID_CC_IN_RPT_LEN       4   // Consumer Control input report Length  2
#define HID_KB_RPT_ID_CC_IN     0x02    //keyboard Control input report ID
#define HID_KB_CC_IN_RPT_LEN    8       //keyboard Control input report Length

void esp_hidd_send_consumer_value(uint8_t key_cmd, bool key_pressed) {
    uint8_t buffer[HID_KB_CC_IN_RPT_LEN] = {0, 0};
    ...
    buffer[6] = 0;
    buffer[7] = 0;
    esp_hidd_dev_input_set(s_ble_hid_param.hid_dev, 1, HID_KB_RPT_ID_CC_IN, buffer, HID_KB_CC_IN_RPT_LEN);
...
void app_main(void) {
    ...
    ESP_ERROR_CHECK(
            esp_hidd_dev_init(&ble_hid_config, ESP_HID_TRANSPORT_BLE, ble_hidd_event_callback,
